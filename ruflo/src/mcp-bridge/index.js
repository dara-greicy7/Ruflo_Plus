...
// =============================================================================
// EXTENSIONS — LangGraph + OpenSpace patterns natively integrated
// (checkpointing · human-in-the-loop · tracing · skill-feedback · cost-tracker)
// =============================================================================
...
// ---------------------------------------------------------------------------
// _coreExecuteTool — pure tool dispatch, no extension logic
// ---------------------------------------------------------------------------
async function _coreExecuteTool(name, args) {
...
// ---------------------------------------------------------------------------
// executeTool — public entry point, wrapped with all extension lifecycle hooks
// ---------------------------------------------------------------------------
async function executeTool(name, args) {
  // Route extension-contributed tools (ext__*) directly to their handlers
  const extTools = extensions.getAdditionalTools();
  const extTool  = extTools.find(t => t.name === name);
  if (extTool && typeof extTool._extensionHandler === "function") {
    try {
      const result = extTool._extensionHandler(args || {});
      return result;
    } catch (err) {
      return { error: `Extension tool error: ${err.message}` };
    }
  }

  // All other tools run through the extension lifecycle (before/after/error hooks)
  return extensions.executeWithExtensions(name, args, _coreExecuteTool);
}
...
async function handleAutopilot(req, res, provider, body) {
  ...
  while (step < maxSteps && !aborted) {
    ...
    try {
      ...
      // Track autopilot token usage for cost governance
      if (aiResult?.usage) extensions.onChatCompletion(body?.model || '', aiResult.usage);
    } catch (err) {
...
app.post("/chat/completions", async (req, res) => {
  ...
  try {
    ...
    if (req.body?.stream && upstream.body) {
      ...
      // Track token usage for cost governance (extension hook)
      try {
        const parsed = JSON.parse(responseText);
        if (parsed?.usage) extensions.onChatCompletion(model, parsed.usage);
      } catch (_) { /* non-JSON or no usage field — skip */ }
...
async function main() {
  // --- Initialize extensions (LangGraph + OpenSpace patterns) ---
  await extensions.initialize();

  // Inject extension-contributed tools into the core tool list so they appear
  // in every tools/list response and can be called by the AI.
  const extensionTools = extensions.getAdditionalTools();
  if (extensionTools.length > 0) {
    BUILTIN_TOOLS.push(...extensionTools);
    console.log(`[Extensions] Injected ${extensionTools.length} tools into BUILTIN_TOOLS: ${extensionTools.map(t => t.name).join(", ")}`);
  }
